...
```
heroku create scheduler_api-prod --remote prod && \
    heroku addons:create newrelic:wayne --app scheduler_api-prod && \
    heroku addons:create heroku-postgresql:hobby-dev --app scheduler_api-prod && \
    heroku config:set DJANGO_SECRET=`openssl rand -base64 32` \
        DJANGO_AWS_ACCESS_KEY_ID="Add your id" \
        DJANGO_AWS_SECRET_ACCESS_KEY="Add your key" \
        DJANGO_AWS_STORAGE_BUCKET_NAME="scheduler_api-prod" \
        --app scheduler_api-prod
```

Creating the qa sever:

```
heroku create `scheduler_api-qa --remote qa && \
    heroku addons:create newrelic:wayne && \
    heroku addons:create heroku-postgresql:hobby-dev && \
    heroku config:set DJANGO_SECRET=`openssl rand -base64 32` \
        DJANGO_AWS_ACCESS_KEY_ID="Add your id" \
        DJANGO_AWS_SECRET_ACCESS_KEY="Add your key" \
        DJANGO_AWS_STORAGE_BUCKET_NAME="scheduler_api-qa" \
```
